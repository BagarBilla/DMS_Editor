...
import { contentControlsOfLayout, paragraphFragmentsOf } from './semantic-records.js';
...
function paragraphLinesIndex(layout: SemanticLayout): Map<string, PlacedLine[]> {
  ...
  const indexFragments = (
    fragments: readonly import('./semantic-records.js').BlockFragmentRecord[],
    pageIndex: number
  ): void => {
    const visit = (
      blocks: readonly import('./semantic-records.js').BlockFragmentRecord[]
    ): void => {
