...
import type { ImageResourceState } from '../store/package/image-resources.js';
import { measureDisplayText } from './run-style.js';
import {
  drawingGeometryFromProjection,
  clipGeometryToRegion,
  type DrawingGeometry,
} from './drawing-geometry.js';
import type { LayoutBox } from './semantic-records.js';

export type { DrawingGeometry } from './drawing-geometry.js';
...
export interface InlineDrawingLayoutContext {
  ...
  readonly project: (
    drawing: import('../store/package/ooxml-tree.js').OoxmlDrawingNode
...
> {
   ...
   */
  readonly textboxStory?: import('./textbox-story-layout.js').TextboxStoryLayout;
}
...
export function buildAnchoredDrawingRecord(options: {
  ...
  readonly sourceOrder?: number;
  readonly textboxStory?: import('./textbox-story-layout.js').TextboxStoryLayout | null;
}): AnchoredDrawingRecord {
  ...
  const visit = (node: OoxmlNode): void => {
    if (node.kind === 'drawing') {
      const projection =
        ...
        context.project(node as import('../store/package/ooxml-tree.js').OoxmlDrawingNode);
...
export function anchorCharacterXOnLine(
  line: {
    ...
    readonly spans: readonly {
      ...
      readonly style?: import('./run-style.js').ResolvedRunStyle;
    }[];
  },
  modelOffset: number,
  measurer?: import('./semantic-records.js').TextMeasurer
): number {
  for (const span of line.spans) {
...
export function publishAnchoredDrawingsForParagraph(options: {
  ...
  readonly measurer?: import('./semantic-records.js').TextMeasurer;
