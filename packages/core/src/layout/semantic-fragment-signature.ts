...
import type { BlockFragmentRecord } from './semantic-records.js';
...
export function sameAnchoredDrawings(
  left: readonly import('./drawing-layout.js').AnchoredDrawingRecord[],
  right: readonly import('./drawing-layout.js').AnchoredDrawingRecord[]
): boolean {
  if (left.length !== right.length) return false;
  for (let index = 0; index < left.length; index += 1) {
