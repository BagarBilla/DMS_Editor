...
import { hardBreakAttributes, hardBreakText } from '../package/hard-break.js';
import { fieldAtomText } from '../package/field-nodes.js';
import { isContentRevisionKind, W14_NAMESPACE_URI } from '../package/ooxml-shared.js';
import {
  WML_NAMESPACE_URI,
  type OoxmlAttribute,
  type OoxmlElement,
...
import {
  ...
  replaceNode,
  type EditOptions,
} from '../package/ooxml-edit.js';
import { wmlFreshNamespaceContextAt } from '../package/wml-namespace.js';
import { resolveRevisions } from './tree-op-revisions.js';
import { applyInsertCommentMarker } from './tree-op-comments.js';
import {
  applyDeleteTracked,
  applyInsertTracked,
...
import {
  ...
  runPropertiesNodeOf,
  sdtPrChild,
} from './tree-op-nodes.js';
import { paragraphIdsWithin, survivingCaretAfterBlockRemoval } from './tree-op-blocks.js';
import {
  PARAGRAPH_VOCABULARY,
  RUN_VOCABULARY,
  mergedPropertyChildren,
} from './tree-op-properties.js';
import {
  applySetListLevel,
...
} from './tree-op-section.js';
import { pageFieldContentBuilders } from './tree-op-fields.js';
import {
...
} from './tree-op-content-controls.js';
import {
  insertionSite,
  isParagraph,
  paragraphLength,
  ...
  segmentsOf,
  type Segment,
} from './tree-op-segments.js';
import {
  applyInsertToc,
  applyReplaceTocResult,
  applyRewriteTocPageNumbers,
} from './tree-op-toc.js';
import { applyInsertTable } from './tree-op-insert-table.js';
import type {
  OoxmlProperty,
  TreeDocOp,
  TreeOpEffect,
...
import { applyDrawingOp, isDrawingTreeDocOp } from './tree-op-drawings.js';
...
function coalesceRunChildren(nextId: () => string, nodes: readonly OoxmlNode[]): OoxmlNode[] {
  ...
  for (const node of nodes) {
    ...
    const left = prevChildren.find((child) => child.kind === 'textValue');
    const right = nodeChildren.find((child) => child.kind === 'textValue');
    const value =
