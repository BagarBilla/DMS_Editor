...
import { createNodeIdAllocator, insertChildren, removeNode } from './ooxml-edit.js';
import { readOoxmlPart, type OoxmlNode } from './ooxml-tree.js';
import type { OoxmlExternalTarget, OoxmlPackage } from './ooxml-package.js';
import { partNameKey, resolveInternalTarget, validateExternalTarget } from './opc-names.js';
import { contentTypesPartBytes } from './package-edit.js';
import type { RelationshipRecord } from './relationships.js';
import { readXml, type XmlNode } from './xml-reader.js';
...
function cloneRelsPart(
  source: import('./ooxml-tree.js').OoxmlPart,
  partName: string
): import('./ooxml-tree.js').OoxmlPart | null {
  let counter = 0;
  const nextId = (): string => {
    counter += 1;
