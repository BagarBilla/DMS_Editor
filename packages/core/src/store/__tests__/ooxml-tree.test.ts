...
describe('drawing semantic digest', () => {
  test('reports drawing-path differences when embed, docPr, or graphic payload children are lost', () => {
    const { diffSemanticDigests, semanticDigest } = engineCore as typeof engineCore & {
      readonly diffSemanticDigests: typeof import('../package/ooxml-digest.js').diffSemanticDigests;
      readonly semanticDigest: typeof import('../package/ooxml-digest.js').semanticDigest;
    };
...
describe('typed drawing fix round 1', () => {
  ...
  test('drawing semantic digest traversal stays bounded at MAX_XML_DEPTH', async () => {
    const { MAX_XML_DEPTH } = await import('../package/xml-reader.js');
    const { semanticDigest } = await import('../package/ooxml-digest.js');
...
describe('comprehensive fixture drawing round trip', () => {
  test('preserves all eleven drawings, empty a:srcRect, and canonical fingerprint', async () => {
    ...
    const { readOoxmlPackage, writeOoxmlPackage } = await import('../package/ooxml-package.js');
...
describe('drawing is one atomic segment', () => {
  test('A + drawing + B uses offsets 0, 1, 2, 3 and deletes exactly at [1, 2)', async () => {
    const { segmentsOf } = await import('../store/tree-op-segments.js');
    const { applyTreeOp, paragraphTextOf } = await import('../store/tree-ops.js');
    const { validateTreeOp } = await import('../store/tree-op-validate.js');
    const { FIELD_ATOM_CHAR } = await import('../package/field-nodes.js');
  ...
  test('run-level MC branch-limit refusal still emits one inert atomic segment', async () => {
    ...
    const { applyTreeOp, paragraphTextOf } = await import('../store/tree-ops.js');
    const { validateTreeOp } = await import('../store/tree-op-validate.js');
    const { resolveRunLevelMcAtom } = await import('../package/drawing-projection.js');
    const { FIELD_ATOM_CHAR } = await import('../package/field-nodes.js');
    const MC = 'http://schemas.openxmlformats.org/markup-compatibility/2006';
