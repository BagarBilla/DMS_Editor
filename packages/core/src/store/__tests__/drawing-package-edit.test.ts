...
import { sha256FontBytes } from '../../layout/font-resource.js';
import {
...
} from '../package/drawing-package-edit.js';
import {
  readOoxmlPackage,
  writeOoxmlPackage,
  type OoxmlPackage,
} from '../package/ooxml-package.js';
import {
  contentTypesPartBytes,
  relationshipsOf,
  resolveContentTypeOf,
} from '../package/package-edit.js';
import { IMAGE_RELATIONSHIP_TYPE } from '../package/relationships.js';
import { imageResourceLookupFor } from '../package/image-resources.js';
import { normalizePartName, partNameKey, resolveInternalTarget } from '../package/opc-names.js';
import {
...
describe('inserts an image atomically', () => {
  test('commits one event, one history entry, media, rel, override, and docPr', async () => {
    const { TreePackageStore } = await import('../store/tree-package-store.js');
    const { semanticDigest, diffSemanticDigests } = await import('../package/ooxml-digest.js');
    const pkg = buildPackage({
...
describe('replaces shared and unshared image media', () => {
  test('keeps shared bytes until the last reference is replaced', async () => {
    const { TreePackageStore } = await import('../store/tree-package-store.js');
    const { readOoxmlPart } = await import('../package/ooxml-tree.js');
    const sharedXml =
...
describe('embeds external image only after explicit command', () => {
  ...
  test('embeds after successful fetch with manual redirect hop', async () => {
    ...
    } = await import('../package/drawing-projection.js');
    const linkedXml =
      `<w:document xmlns:w="${W}" xmlns:wp="${WP}" xmlns:a="${A}" xmlns:pic="${PIC}" xmlns:r="${R}">` +
      '<w:body><w:p><w:r><w:drawing><wp:inline distT="0" distB="0" distL="0" distR="0">' +
      '<wp:extent cx="12700" cy="12700"/><wp:docPr id="1" name=""/>' +
    ...
    const projection = projectDrawing(
      after.parts
        ...
        .children.find(
          (c) => c.kind === 'drawing'
        )! as import('../package/ooxml-tree.js').OoxmlDrawingNode,
      {
...
describe('fetchExternalImageBytes', () => {
  test('refuses content-type spoof and byte overflow without publishing', async () => {
    const { fetchExternalImageBytes } = await import('../package/drawing-package-edit.js');
    const { resolveImageResourceLimits } = await import('../runtime/limits.js');
...
describe('creates drawing hyperlink in package transaction', () => {
  test('setDrawingMetadataWithHyperlink adds external rel and hlinkClick', async () => {
    const { TreePackageStore } = await import('../store/tree-package-store.js');
    const { relationshipTargetIn } = await import('../package/hyperlink-part.js');
    const { HYPERLINK_RELATIONSHIP_TYPE } = await import('../package/hyperlink.js');
    const pkg = buildPackage({
      document: `<w:document xmlns:w="${W}"><w:body><w:p><w:r></w:r></w:p></w:body></w:document>`,
    });
    const main = pkg.parts.get('/word/document.xml')!;
