...
import { readOoxmlPackage } from '../package/ooxml-package.js';
...
describe('package writer (cutover step 2)', () => {
  test('an unedited package round-trips through both D9 oracles', async () => {
    const { canonicalOoxmlFingerprint } = await import('../package/ooxml-tree.js');
    const { semanticDigest, diffSemanticDigests } = await import('../package/ooxml-digest.js');
    const { writeOoxmlPackage } = await import('../package/ooxml-package.js');
  ...
  test('an edited tree is what gets written', async () => {
    const { withPart, writeOoxmlPackage } = await import('../package/ooxml-package.js');
    const { applyTreeOp } = await import('../store/tree-ops.js');
    const { deriveOoxmlIndexes } = await import('../package/ooxml-indexes.js');
  ...
  test('mc:Ignorable and AlternateContent survive package save/reopen', async () => {
    const { canonicalOoxmlFingerprint, serializeOoxmlPart } =
      await import('../package/ooxml-tree.js');
    const { writeOoxmlPackage } = await import('../package/ooxml-package.js');
...
describe('drawing resources', () => {
  test('resolves embedded media through owner-relative relationships without fetch', async () => {
    const { createImageResourceCache } = await import('../package/image-resources.js');
    const A = 'http://schemas.openxmlformats.org/drawingml/2006/main';
    const WP = 'http://schemas.openxmlformats.org/drawingml/2006/wordprocessingDrawing';
