...
import { enumerateDocumentSections, paragraphSectionNode } from '../layout/section-properties.js';
import { storyBlocks } from '../layout/story-roots.js';
import {
...
): readonly (import('../store/package/ooxml-tree.js').OoxmlElement | undefined)[] {
  const part = session.part();
  const blocks = storyBlocks(part);
  const nodes: (import('../store/package/ooxml-tree.js').OoxmlElement | undefined)[] = [];
