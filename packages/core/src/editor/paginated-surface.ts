...
import { syncActiveFieldShading } from './surface-field-shading.js';
import {
...
} from '../output/semantic-paint-drawings.js';
import { tryCreateBrowserCanvasContext } from './browser-canvas-context.js';
import type {
  ContentControlOps,
  OpenPaginatedResult,
  PaginatedSurface,
...
} from './surface-selection-ops.js';
import {
  createBeforeInputHandler,
  createClipboardHandlers,
  createKeyDownHandler,
} from './surface-input.js';
import { insertableText } from './clipboard-plain-text.js';
import {
  ...
  viewportPage,
  type SurfaceExtent,
} from './surface-pages.js';
import {
  tryCreateBrowserImageDecodePort,
  createHeadlessImageDecodePort,
} from './browser-image-decode-port.js';
import { createBrowserPaintImageUrlPort } from './browser-paint-image-url-port.js';
import { createInlineDrawingLayoutBundle } from '../layout/inline-drawing-source.js';
import { createSurfaceCaret } from './surface-caret.js';
import { defaultTableLabel, type TableInteractionLabelKey } from './table-chrome.js';
import { createSurfaceTableInteraction } from './surface-table-interaction.js';
import { createSurfaceFormat } from './surface-format.js';
import {
  authoredRunPropertiesAt,
  mergedProperties,
  type SurfaceProperty,
} from './surface-formatting.js';
import { createPointerController, type PointerController } from './surface-pointer.js';
import { createSurfaceSelectionSync } from './surface-selection-sync.js';
import { createSurfaceStructure } from './surface-structure.js';
...
import { MIN_TABLE_COLUMN_WIDTH_TWIPS } from '../store/store/table-constraints.js';
import { createHyperlinkOps } from './surface-hyperlinks.js';
import { createSurfaceNavigation } from './surface-navigation.js';
import { drawingLinkByIdFromLayout } from './drawing-link-index.js';
import {
  furnitureCaretHost,
...
import { createHeaderFooterOps } from './surface-hf-ops.js';
import { createImageOps } from './surface-image-ops.js';
import { createHeaderFooterScopeController } from './surface-hf-editing.js';
import { createNoteOps } from './surface-note-ops.js';
import { notePropertiesStateOf, notePreviewTextOf } from './surface-note-state.js';
...
export function mountPaginatedSurface(
  ...
  try {
    const findElement = (node: OoxmlNode, predicate: (el: OoxmlElement) => boolean): OoxmlElement | null => {
    ...
    const getAttr = (el: OoxmlElement, attrName: string): string | undefined => {
      return el.attributes.find((a) => a.localName === attrName)?.value;
    };
