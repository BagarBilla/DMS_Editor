...
import { paragraphOrderOfPart, type ReviewItem } from '../layout/review-support.js';
import {
  canApplyLocalReviewPatch,
  localReviewPatchParagraphId,
  patchLocalReviewItems,
} from './review-patch.js';
import type { ReviewModuleContribution } from '../contracts/modules.js';
import {
  addComment,
  setCommentResolved,
  commentPartNameOf,
  commentsExtendedPartNameOf,
} from '../store/store/comment-writes.js';
import {
...
} from '../store/store/custom-node-writes.js';
import { deleteCommentThread } from '../store/package/comment-lifecycle.js';
import { resolveNotesPart } from '../store/package/note-references.js';
import {
  ORIGIN_IDS,
  TreePackageStore,
  readEmbeddedFonts,
...
import {
  ...
  reconcileDoc,
  treeToDoc,
} from './tree-binding.js';
import type { TreeBindingRejection } from './tree-binding.js';
import { buildParagraphAnchorIndex, type ParagraphAnchorIndex } from './paragraph-anchors.js';
import {
  readTrackingSettings,
  type DocumentTrackingSettings,
} from '../store/package/tracking-settings.js';
...
export interface TreeDocxSession {
  ...
  insertImage(
    scope: StoryScope,
    input: import('../store/store/tree-package-images.js').InsertImageInput
  ): Promise<import('../store/store/tree-package-images.js').ImageIntentResult>;
  ...
  replaceImage(
    ...
    bytes: Uint8Array,
    mime: import('../store/package/image-resources.js').SupportedImageMime,
    decodePort: import('../store/package/image-resources.js').ImageDecodePort,
    options: import('../store/store/tree-package-images.js').ReplaceImageOptions
  ): Promise<import('../store/store/tree-package-images.js').ImageIntentResult>;
  ...
  deleteImage(
    scope: StoryScope,
    drawingNodeId: string
  ): import('../store/store/tree-package-images.js').ImageIntentResult;

  /** Apply image property tree ops plus hyperlink relationship wiring atomically. */
  applyImageProperties(
    scope: StoryScope,
    input: import('../store/store/tree-package-images.js').ApplyImagePropertiesInput
  ): import('../store/store/tree-package-images.js').ImageIntentResult;
}

export type { DocumentStyleEntry } from './document-catalog.js';
export type { DocumentThemeColorEntry, ThemeColorSlot } from './document-theme.js';
export type { DocumentOutlineEntry } from './document-outline.js';
export type { ParagraphAnchorIndex } from './paragraph-anchors.js';
