//#region ../../node_modules/.bun/emf-converter@2.0.2/node_modules/emf-converter/dist/index.mjs
function colorRefToHex(r, g, b) {
	const toHex = (v) => v.toString(16).padStart(2, "0");
	return `#${toHex(r & 255)}${toHex(g & 255)}${toHex(b & 255)}`;
}
function readColorRef(view, offset) {
	return colorRefToHex(view.getUint8(offset), view.getUint8(offset + 1), view.getUint8(offset + 2));
}
function argbToRgba(argb) {
	const a = (argb >>> 24 & 255) / 255;
	return `rgba(${argb >>> 16 & 255},${argb >>> 8 & 255},${argb & 255},${a.toFixed(3)})`;
}
function lerpArgbToRgba(argbA, argbB, t) {
	...
	return `rgba(${mix(argbA >>> 16 & 255, argbB >>> 16 & 255)},${mix(argbA >>> 8 & 255, argbB >>> 8 & 255)},${mix(argbA & 255, argbB & 255)},${(mix(aA, aB) / 255).toFixed(3)})`;
}
function invertCssColor(color) {
	const hex = /^#([0-9a-f]{6})$/i.exec(color);
	if (hex) return `#${(16777215 ^ parseInt(hex[1], 16)).toString(16).padStart(6, "0")}`;
...
function createCanvas(width, height, maxWidth, maxHeight, dpiScale = 1, maxCanvasDimension = MAX_CANVAS_DIMENSION) {
	...
	if (clampedW !== w || clampedH !== h) console.warn(`[emf-converter] Canvas size clamped from ${w}\xD7${h} to ${clampedW}\xD7${clampedH}. Output may lose detail.`);
	w = clampedW;
	h = clampedH;
	try {
		if (typeof OffscreenCanvas !== "undefined") {
			emfLog(`createCanvas: using OffscreenCanvas ${w}\xD7${h}, scale=(${scaleX.toFixed(3)},${scaleY.toFixed(3)})`);
		...
		emfLog(`createCanvas: using HTMLCanvasElement ${w}\xD7${h}, scale=(${scaleX.toFixed(3)},${scaleY.toFixed(3)})`);
...
function rop2Paint(rop2) {
	switch (rop2) {
		case R2_BLACK: return {
			gco: "source-over",
			colorTransform: "black",
			exact: true
		};
		case R2_WHITE: return {
			gco: "source-over",
		...
		case R2_XORPEN:
		case R2_MASKPENNOT: return {
			gco: "difference",
			colorTransform: "none",
			exact: false
		};
		case R2_NOTXORPEN: return {
			gco: "difference",
			colorTransform: "invert",
			exact: false
		};
		case R2_MASKPEN: return {
			gco: "darken",
			colorTransform: "none",
			exact: false
		};
		case R2_MASKNOTPEN:
		case R2_NOTMERGEPEN: return {
			gco: "darken",
			colorTransform: "invert",
			exact: false
		};
		case R2_MERGEPEN:
		case R2_MERGEPENNOT: return {
			gco: "lighten",
			colorTransform: "none",
			exact: false
		};
		case R2_MERGENOTPEN:
		case R2_NOTMASKPEN: return {
			gco: "lighten",
			colorTransform: "invert",
			exact: false
		};
...
function rop2TransformColor(color, transform) {
	switch (transform) {
		case "invert": return invertCssColor(color);
		case "black": return "#000000";
		case "white": return "#ffffff";
		case "skip": return "rgba(0,0,0,0)";
...
function cssFontWeight(weight) {
	...
	if (rounded === 700) return "bold";
	if (rounded >= 100 && rounded <= 900) return String(rounded);
	return weight >= 700 ? "bold" : "";
}
function mapFontFamily(face, map) {
	const resolved = map?.[face.toLowerCase().trim()] ?? face;
	if (/[\s,]/.test(resolved) && !/^["']/.test(resolved)) return `"${resolved}"`;
	return resolved;
}
...
function applyFont(ctx, state, scale = 1) {
	...
	ctx.font = `${italic}${weight ? `${weight} ` : ""}${fontSizePx(state, scale)}px ${mapFontFamily(state.fontFamily, state.fontFamilyMap)}`;
}
function drawTextDecorations(ctx, state, x, y, width, scale = 1) {
	if (!state.fontUnderline && !state.fontStrikeOut) return;
	const size = fontSizePx(state, scale);
	const thickness = Math.max(1, Math.round(size / 14));
	const prevFill = ctx.fillStyle;
	ctx.fillStyle = state.textColor;
	if (state.fontUnderline) ctx.fillRect(x, y + Math.round(size * .12), width, thickness);
	if (state.fontStrikeOut) ctx.fillRect(x, y - Math.round(size * .3), width, thickness);
...
async function exportCanvasToPngDataUrl(canvas) {
	if (typeof OffscreenCanvas !== "undefined" && canvas instanceof OffscreenCanvas) {
		emfLog(`exportCanvasToPngDataUrl: using OffscreenCanvas.convertToBlob (${canvas.width}\xD7${canvas.height})`);
		const blob = await canvas.convertToBlob({ type: "image/png" });
		emfLog(`exportCanvasToPngDataUrl: blob size=${blob.size} bytes, type=${blob.type}`);
		return blobToDataUrl(blob);
	}
	if (typeof HTMLCanvasElement !== "undefined" && canvas instanceof HTMLCanvasElement) {
		emfLog(`exportCanvasToPngDataUrl: using HTMLCanvasElement.toDataURL (${canvas.width}\xD7${canvas.height})`);
...
function parseEmfHeader(view) {
	...
	if (view.getUint32(0, true) !== EMR_HEADER) return null;
...
function getRenderableEmfBounds(header) {
	...
	if (boundsW > 0 && boundsH > 0) return header.bounds;
	if (header.frameW > 0 && header.frameH > 0) {
		emfLog(`getRenderableEmfBounds: bounds invalid (${boundsW}\xD7${boundsH}), falling back to frame ${header.frameW}\xD7${header.frameH}`);
		return {
			left: 0,
			top: 0,
			right: header.frameW,
			bottom: header.frameH
		};
	}
	return null;
}
...
function gmx(r, x) {
	...
	if (r.useMappingMode) return (px - r.windowOrg.x) / (r.windowExt.cx || 1) * (r.viewportExt.cx || 1) + r.viewportOrg.x;
	return (px - r.bounds.left) * r.sx;
}
function gmy(r, y) {
	const wt = r.state.worldTransform;
	const py = wt[3] * y + wt[5];
	if (r.useMappingMode) return (py - r.windowOrg.y) / (r.windowExt.cy || 1) * (r.viewportExt.cy || 1) + r.viewportOrg.y;
	return (py - r.bounds.top) * r.sy;
}
function gmw(r, w) {
	const pw = r.state.worldTransform[0] * w;
	if (r.useMappingMode) return pw / (r.windowExt.cx || 1) * (r.viewportExt.cx || 1);
	return pw * r.sx;
}
function gmh(r, h) {
	const ph = r.state.worldTransform[3] * h;
	if (r.useMappingMode) return ph / (r.windowExt.cy || 1) * (r.viewportExt.cy || 1);
	return ph * r.sy;
}
...
function handleSetPixelV(rCtx, dataOff, recSize) {
	...
	if (recSize >= 20) {
		...
		ctx.fillStyle = readColorRef(view, dataOff + 8);
...
function handleLineTo(rCtx, dataOff, recSize) {
	...
	if (recSize >= 16) {
		...
		if (inPath) ctx.lineTo(gmx(rCtx, lx), gmy(rCtx, ly));
		else {
			applyPen(ctx, state);
			ctx.beginPath();
...
function handleRectangle(rCtx, dataOff, recSize) {
	...
	if (recSize >= 24) {
		...
		if (inPath) ctx.rect(gmx(rCtx, l), gmy(rCtx, t), gmw(rCtx, r - l), gmh(rCtx, b - t));
		else {
...
function handleEllipse(rCtx, dataOff, recSize) {
	...
	if (recSize >= 24) {
		...
		if (inPath) ctx.ellipse(cx, cy, rx, ry, 0, 0, Math.PI * 2);
		else {
			ctx.beginPath();
...
function handleArcFamily(rCtx, recType, dataOff, recSize) {
	...
	if (recSize >= 40) {
		...
		if (isArcTo) ctx.lineTo(mcx + mrx * Math.cos(startAngle), mcy + mry * Math.sin(startAngle));
...
function handleEmfGdiShapeRecord(rCtx, recType, dataOff, recSize) {
	switch (recType) {
		case EMR_SETPIXELV: return handleSetPixelV(rCtx, dataOff, recSize);
		case EMR_MOVETOEX: return handleMoveToEx(rCtx, dataOff, recSize);
		case EMR_LINETO: return handleLineTo(rCtx, dataOff, recSize);
		case EMR_RECTANGLE: return handleRectangle(rCtx, dataOff, recSize);
		case EMR_ROUNDRECT: return handleRoundRect(rCtx, dataOff, recSize);
		case EMR_ELLIPSE: return handleEllipse(rCtx, dataOff, recSize);
		case EMR_ARC:
		case EMR_ARCTO:
		case EMR_CHORD:
		case EMR_PIE: return handleArcFamily(rCtx, recType, dataOff, recSize);
...
function combineClip(current, shape, op) {
	switch (op) {
		...
		case "intersect": return {
			region: current ? [...current, shape] : [shape],
			exact: true
		};
		case "exclude": {
			if (!isComposable(shape)) return {
				region: current ? [...current, shape] : [shape],
				exact: false
			};
			const inv = invertClipShape(shape);
			return {
				region: current ? [...current, inv] : [inv],
				exact: true
			};
		}
		case "union":
			...
			if (current.length === 1 && isComposable(current[0]) && isComposable(shape)) return {
				region: [{
					cmds: [...current[0].cmds, ...shape.cmds],
					fillRule: "nonzero",
					simple: false
				}],
				exact: false
		...
		case "xor":
			...
			if (isComposable(shape)) return {
				region: [...current, invertClipShape(shape)],
				exact: false
			};
			return {
				region: [...current, shape],
				exact: false
			};
		case "complement":
			if (!current) return {
				region: [emptyClipShape()],
				exact: true
			};
			if (current.length === 1 && isComposable(current[0])) return {
				region: [shape, invertClipShape(current[0])],
				exact: true
			};
			return {
				region: [shape],
...
function combineClipRegions(current, incoming, op) {
	...
	if (incoming && incoming.length === 1) return combineClip(current, incoming[0], op);
	if (!incoming) switch (op) {
		case "intersect": return {
			region: current,
			exact: true
		};
		...
		case "complement":
			...
			if (current.length === 1) return combineClip(null, current[0], "exclude");
			return {
				region: current,
				exact: false
			};
	...
	switch (op) {
		case "intersect": return {
			region: current ? [...current, ...incoming] : incoming,
			exact: true
		};
		case "union": return {
			region: current,
		...
		case "complement":
			...
			if (current.length === 1) return combineClip(incoming, current[0], "exclude");
			return {
				region: incoming,
				exact: false
			};
...
function replayClipCmds(ctx, cmds) {
	for (const c of cmds) switch (c.op) {
		case "rect":
...
function reapplyClipRegion(holder, region, identityTransform = false) {
	...
	if (region) {
		...
		if (identityTransform) ctx.setTransform(1, 0, 0, 1, 0, 0);
...
function decodeRleBitmap(view, bitsOffset, bitsSize, width, height, _topDown, isRle4, colorTable, out, setPixel) {
		...
		if (first === 0) if (second === 0) {
			...
			if (!isRle4) {
				for (let i = 0; i < count && off < endOff && x < width; i++) {
					...
					if (idx < colorTable.length) setPixel(x, height - 1 - y, colorTable[idx][0], colorTable[idx][1], colorTable[idx][2], 255);
					x++;
				}
				if (count & 1) off++;
			} else {
				...
				for (let i = 0; i < bytes && off < endOff; i++) {
					...
					for (let nibble = 0; nibble < 2 && pi < count; nibble++) {
						...
						if (idx < colorTable.length && x < width) setPixel(x, height - 1 - y, colorTable[idx][0], colorTable[idx][1], colorTable[idx][2], 255);
						x++;
						pi++;
					}
				}
			...
			const idx = second;
			const c = idx < colorTable.length ? colorTable[idx] : [
				0,
				0,
				0
			];
	...
	return new ImageData(new Uint8ClampedArray(out.buffer, out.byteOffset, out.byteLength), width, height);
}
...
function decodeUncompressedRows(view, bitsOffset, width, height, topDown, bitCount, colorTable, masks, out) {
	...
	for (let y = 0; y < height; y++) {
		const rowStart = bitsOffset + (topDown ? y : height - 1 - y) * rowStride;
...
function decodeDibToImageData(view, bmiOffset, bitsOffset, bitsSize) {
	if (bmiOffset < 0 || bitsOffset < 0 || bmiOffset + 40 > view.byteLength || bitsOffset + bitsSize > view.byteLength) return null;
	...
	if (bitCount !== 1 && bitCount !== 4 && bitCount !== 8 && bitCount !== 16 && bitCount !== 24 && bitCount !== 32) return null;
	if (compression === BI_RLE8 && bitCount !== 8) return null;
	if (compression === BI_RLE4 && bitCount !== 4) return null;
	if (compression === BI_BITFIELDS2 && bitCount !== 16 && bitCount !== 32) return null;
	if (compression !== BI_RGB && compression !== BI_RLE8 && compression !== BI_RLE4 && compression !== BI_BITFIELDS2) return null;
	...
	if (compression === BI_RLE8 || compression === BI_RLE4) {
		...
		return decodeRleBitmap(view, bitsOffset, bitsSize, width, height, topDown, compression === BI_RLE4, colorTable, out, setPixel);
	}
	decodeUncompressedRows(view, bitsOffset, width, height, topDown, bitCount, colorTable, masks, out);
	...
	if (recSize >= 76) {
		...
		if (nChars > 0 && offString > 0 && offset + offString + nChars * 2 <= maxOffset) {
			...
			if (text.length > 0) {
				...
				let alignHoriz = "left";
				if (state.textAlign & 6) alignHoriz = "center";
				if (state.textAlign & 2) alignHoriz = "right";
				...
				if (state.fontUnderline || state.fontStrikeOut) {
					...
					drawTextDecorations(ctx, state, alignHoriz === "center" ? baseX - w / 2 : alignHoriz === "right" ? baseX - w : baseX, gmy(rCtx, refY), w, fontScale);
				}
			}
		}
	}
...
function handleBitBlt(rCtx, offset, dataOff, recSize) {
	...
	if (recSize >= 96) {
		...
		if (offBmiSrc > 0 && cbBmiSrc > 0 && offBitsSrc > 0 && cbBitsSrc > 0) {
			const imageData = decodeDibToImageData(view, offset + offBmiSrc, offset + offBitsSrc, cbBitsSrc);
			if (imageData) {
				const temp = createTempCanvas(imageData.width, imageData.height);
				if (temp) {
					temp.ctx.putImageData(imageData, 0, 0);
					ctx.drawImage(temp.canvas, gmx(rCtx, dstX), gmy(rCtx, dstY), gmw(rCtx, dstW), gmh(rCtx, dstH));
				}
			}
		}
	}
...
function gdiCombineClip(rCtx, shape, op) {
	const { ctx } = rCtx;
	if (rCtx.clipUntracked && op !== "replace") switch (op) {
		case "intersect":
		case "complement":
			ctx.save();
			rCtx.clipSaveDepth++;
...
function readClipRectShape(rCtx, dataOff) {
	...
	return rectClipShape(gmx(rCtx, left), gmy(rCtx, top), gmw(rCtx, right - left), gmh(rCtx, bottom - top));
}
function handleIntersectClipRect(rCtx, dataOff, recSize) {
	if (recSize >= 24) gdiCombineClip(rCtx, readClipRectShape(rCtx, dataOff), "intersect");
	return true;
}
function handleExcludeClipRect(rCtx, dataOff, recSize) {
	if (recSize >= 24) gdiCombineClip(rCtx, readClipRectShape(rCtx, dataOff), "exclude");
	return true;
}
var RGN_MODE_OPS = {
	1: "intersect",
...
function handleExtSelectClipRgn(rCtx, dataOff, recSize) {
	...
	const op = RGN_MODE_OPS[view.getUint32(dataOff + 4, true)];
	if (!op) return true;
	if (cbRgnData === 0) {
		if (op === "replace") {
...
function handleOffsetClipRgn(rCtx, dataOff, recSize) {
	if (recSize >= 16) {
		...
		if (rCtx.clipUntracked) return true;
		if (rCtx.clipRegion) {
...
function handleEmfGdiTextBitmapRecord(rCtx, recType, offset, dataOff, recSize) {
	switch (recType) {
		case EMR_EXTTEXTOUTW: return handleExtTextOutW(rCtx, offset, dataOff, recSize);
		case EMR_BITBLT: return handleBitBlt(rCtx, offset, dataOff, recSize);
		case EMR_STRETCHDIBITS: return handleStretchDibits(rCtx, offset, dataOff, recSize);
		case EMR_INTERSECTCLIPRECT: return handleIntersectClipRect(rCtx, dataOff, recSize);
		case EMR_EXTSELECTCLIPRGN: return handleExtSelectClipRgn(rCtx, dataOff, recSize);
		case EMR_EXCLUDECLIPRECT: return handleExcludeClipRect(rCtx, dataOff, recSize);
		case EMR_OFFSETCLIPRGN: return handleOffsetClipRgn(rCtx, dataOff, recSize);
		default: return false;
	}
}
...
function handlePolyPolygon32(rCtx, offset, dataOff, recSize) {
	...
	if (numPolys === 0 || numPolys >= 1e4 || totalPoints >= 1e5) return;
...
function handlePoly32(rCtx, recType, offset, dataOff, recSize) {
	...
	if (!inPath) ctx.beginPath();
	if (!isTo) ctx.moveTo(gmx(rCtx, view.getInt32(ptOff, true)), gmy(rCtx, view.getInt32(ptOff + 4, true)));
	let i = isTo ? 0 : 1;
	if (isBezier) while (i + 2 < count) {
		ctx.bezierCurveTo(gmx(rCtx, view.getInt32(ptOff + i * 8, true)), gmy(rCtx, view.getInt32(ptOff + i * 8 + 4, true)), gmx(rCtx, view.getInt32(ptOff + (i + 1) * 8, true)), gmy(rCtx, view.getInt32(ptOff + (i + 1) * 8 + 4, true)), gmx(rCtx, view.getInt32(ptOff + (i + 2) * 8, true)), gmy(rCtx, view.getInt32(ptOff + (i + 2) * 8 + 4, true)));
		i += 3;
	}
	else for (; i < count; i++) ctx.lineTo(gmx(rCtx, view.getInt32(ptOff + i * 8, true)), gmy(rCtx, view.getInt32(ptOff + i * 8 + 4, true)));
...
function handlePoly16(rCtx, recType, offset, dataOff, recSize) {
	...
	if (!inPath) ctx.beginPath();
	if (!isTo) ctx.moveTo(gmx(rCtx, view.getInt16(ptOff, true)), gmy(rCtx, view.getInt16(ptOff + 2, true)));
	let i = isTo ? 0 : 1;
	if (isBezier) while (i + 2 < count) {
		ctx.bezierCurveTo(gmx(rCtx, view.getInt16(ptOff + i * 4, true)), gmy(rCtx, view.getInt16(ptOff + i * 4 + 2, true)), gmx(rCtx, view.getInt16(ptOff + (i + 1) * 4, true)), gmy(rCtx, view.getInt16(ptOff + (i + 1) * 4 + 2, true)), gmx(rCtx, view.getInt16(ptOff + (i + 2) * 4, true)), gmy(rCtx, view.getInt16(ptOff + (i + 2) * 4 + 2, true)));
		i += 3;
	}
	else for (; i < count; i++) ctx.lineTo(gmx(rCtx, view.getInt16(ptOff + i * 4, true)), gmy(rCtx, view.getInt16(ptOff + i * 4 + 2, true)));
	if (isPolygon) ctx.closePath();
	if (!inPath) {
		if (isPolygon) {
			applyBrush(ctx, state);
...
function handleEmfGdiPolyPathRecord(rCtx, recType, offset, dataOff, recSize) {
	...
	switch (recType) {
		...
		case EMR_POLYLINETO: return handlePoly32(rCtx, recType, offset, dataOff, recSize);
		case EMR_POLYLINE16:
		case EMR_POLYGON16:
		case EMR_POLYBEZIER16:
		case EMR_POLYBEZIERTO16:
		case EMR_POLYLINETO16: return handlePoly16(rCtx, recType, offset, dataOff, recSize);
		...
		case EMR_SELECTCLIPPATH: {
			...
			try {
				if (clipMode === 5) while (rCtx.clipSaveDepth > 0) {
					ctx.restore();
					rCtx.clipSaveDepth--;
				}
				ctx.save();
...
function handleEmfObjectRecord(rCtx, recType, dataOff, recSize) {
	...
	switch (recType) {
		...
		case EMR_DELETEOBJECT:
			if (recSize >= 12) rCtx.objectTable.delete(view.getUint32(dataOff, true));
			return true;
		default: return false;
	}
}
	...
	switch (recType) {
		...
		case EMR_SETMAPMODE:
			if (recSize >= 12) {
				...
				if (mode === 8 || mode === 7) activateGdiMappingMode(rCtx);
			}
			return true;
		case EMR_SCALEVIEWPORTEXTEX:
			if (recSize >= 24) {
				...
				if (xDenom !== 0) rCtx.viewportExt.cx = Math.round(rCtx.viewportExt.cx * xNum / xDenom);
				if (yDenom !== 0) rCtx.viewportExt.cy = Math.round(rCtx.viewportExt.cy * yNum / yDenom);
				activateGdiMappingMode(rCtx);
			}
			return true;
		case EMR_SCALEWINDOWEXTEX:
			if (recSize >= 24) {
				...
				const yNum = view.getInt32(dataOff + 8, true);
				const yDenom = view.getInt32(dataOff + 12, true);
				if (xDenom !== 0) rCtx.windowExt.cx = Math.round(rCtx.windowExt.cx * xNum / xDenom);
				if (yDenom !== 0) rCtx.windowExt.cy = Math.round(rCtx.windowExt.cy * yNum / yDenom);
				activateGdiMappingMode(rCtx);
			}
			return true;
		default: return false;
...
function handleWorldTransformRecord(rCtx, recType, dataOff, recSize) {
	...
	switch (recType) {
		case EMR_SETWORLDTRANSFORM:
			if (recSize >= 32) state.worldTransform = [
		...
		case EMR_MODIFYWORLDTRANSFORM:
			if (recSize >= 36) {
				...
				if (mode === 1) state.worldTransform = [
					1,
					0,
					0,
					1,
					...
					if (mode === 2) state.worldTransform = [
						xf[0] * a1 + xf[1] * c1,
						xf[0] * b1 + xf[1] * d1,
						xf[2] * a1 + xf[3] * c1,
						xf[2] * b1 + xf[3] * d1,
	...
	switch (recType) {
		case EMR_SAVEDC:
			...
			if (rCtx.clipUntracked) {
				rCtx.clipUntracked = false;
				rCtx.clipRegion = null;
			} else if (rCtx.clipRegion) reapplyClipRegion(rCtx, rCtx.clipRegion);
			return true;
		case EMR_RESTOREDC:
			if (recSize >= 12) {
				while (rCtx.clipSaveDepth > 0) {
				...
				if (rel < 0) rel = rCtx.stateStack.length + rel + 1;
				...
				if (restored) {
					...
					rCtx.clipUntracked = false;
					if (rCtx.clipRegion) reapplyClipRegion(rCtx, rCtx.clipRegion);
				}
			}
			return true;
		...
		case EMR_SETPOLYFILLMODE:
			...
			return true;
		case EMR_SETROP2:
			if (recSize >= 12) state.rop2 = view.getUint32(dataOff, true);
			return true;
		case EMR_SETSTRETCHBLTMODE:
		case EMR_SETMITERLIMIT:
		case EMR_SETTEXTALIGN:
			if (recType === EMR_SETTEXTALIGN && recSize >= 12) state.textAlign = view.getUint32(dataOff, true);
			return true;
		default: return false;
	}
}
...
function parseEmfPlusPath(data, off, maxLen) {
	...
	if (pointCount === 0 || pointCount > 1e5) return null;
	const compressed = (pathFlags & 16384) !== 0;
	if (12 + (pointCount * (compressed ? 4 : 8) + pointCount) > maxLen) return null;
	const points = [];
	let pOff = off + 12;
	for (let i = 0; i < pointCount; i++) if (compressed) {
		points.push({
...
function emfPlusPathToClipCmds(path, m) {
	...
	while (i < pts.length) {
		...
		if (t === 0) {
			cmds.push({
				op: "moveTo",
				x: tx(pts[i].x, pts[i].y),
				y: ty(pts[i].x, pts[i].y)
			});
			i++;
		} else if (t === 3) {
			if (i + 2 < pts.length) {
				...
				if ((types[i + 2] & 128) !== 0) cmds.push({ op: "closePath" });
				i += 3;
				continue;
			}
			break;
		...
		if (close) cmds.push({ op: "closePath" });
	}
	return cmds;
}
...
function createBrushGradient(rCtx, grad) {
	...
	try {
		...
		if (grad.type === "linear" && typeof ctx.createLinearGradient === "function") {
			if (grad.x1 === grad.x2 && grad.y1 === grad.y2) return null;
...
function resolveBrushPaint(rCtx, flags, brushIdOrColor) {
	if (flags & 32768) return argbToRgba(brushIdOrColor);
...
function applyPlusWorldTransform(rCtx) {
	...
	rCtx.ctx.setTransform(wt[0] * m * d, wt[1] * m * d, wt[2] * m * d, wt[3] * m * d, wt[4] * m * d, wt[5] * m * d);
}
function pushState(rCtx, stackId) {
	rCtx.saveStack.push({ transform: [...rCtx.worldTransform] });
...
function popState(rCtx, stackId) {
	...
	if (idx !== void 0 && idx < rCtx.saveStack.length) {
		...
		for (const [k, v] of rCtx.saveIdMap) if (v < idx) newMap.set(k, v);
...
function pathClipShape(path, m) {
	return {
		cmds: emfPlusPathToClipCmds(path.path, m),
		fillRule: "nonzero",
		simple: true
	};
}
...
function flattenRegionNode(node, m, depth = 0) {
	if (depth > MAX_REGION_FLATTEN_DEPTH) return {
		region: [emptyClipShape()],
		exact: false
	};
	switch (node.type) {
		case "rect": return {
			region: [transformedRectShape(node.x, node.y, node.width, node.height, m)],
			exact: true
		};
		case "path": return {
			region: [pathClipShape(node, m)],
			exact: true
		};
		...
		case "combine": {
			...
			return {
				...
				exact: combined.exact && left.exact && right.exact
			};
		}
	}
}
...
function handleEmfPlusStateRecord(rCtx, recType, recFlags, dataOff, recDataSize) {
	...
	switch (recType) {
		case EMFPLUS_SETWORLDTRANSFORM:
			if (recDataSize >= 24) rCtx.worldTransform = [
				view.getFloat32(dataOff, true),
				view.getFloat32(dataOff + 4, true),
				view.getFloat32(dataOff + 8, true),
				view.getFloat32(dataOff + 12, true),
		...
		case EMFPLUS_MULTIPLYWORLDTRANSFORM:
			if (recDataSize >= 24) {
				...
				if (recFlags & 8192) rCtx.worldTransform = multiplyMatrix(rCtx.worldTransform, xf);
				else rCtx.worldTransform = multiplyMatrix(xf, rCtx.worldTransform);
			}
			return true;
		...
		case EMFPLUS_ROTATEWORLDTRANSFORM:
			...
			return true;
		case EMFPLUS_SAVE:
			if (recDataSize >= 4) pushState(rCtx, view.getUint32(dataOff, true));
			return true;
		case EMFPLUS_RESTORE:
			if (recDataSize >= 4) popState(rCtx, view.getUint32(dataOff, true));
			return true;
		case EMFPLUS_SETCLIPRECT:
			if (recDataSize >= 16) {
				const combineMode = recFlags >> 8 & 15;
				applyPlusClipShape(rCtx, transformedRectShape(view.getFloat32(dataOff, true), view.getFloat32(dataOff + 4, true), view.getFloat32(dataOff + 8, true), view.getFloat32(dataOff + 12, true), plusDeviceMatrix(rCtx)), combineMode);
			}
			return true;
		...
		case EMFPLUS_SETCLIPPATH: {
			...
			if (pathObj && pathObj.kind === "plus-path") applyPlusClipShape(rCtx, {
		...
		case EMFPLUS_SETCOMPOSITINGQUALITY: return true;
		default: return false;
	}
}
	...
	switch (recType) {
		...
		case EMFPLUS_DRAWRECTS:
			if (recDataSize >= 4) {
				...
				const compressed = (recFlags & 16384) !== 0;
				const rectSize = compressed ? 8 : 16;
				if (pen && pen.kind === "plus-pen") applyEmfPlusPen(ctx, pen);
				applyPlusWorldTransform(rCtx);
				let rOff = dataOff + 4;
				for (let i = 0; i < count && rOff + rectSize <= dataOff + recDataSize; i++) {
					const { x, y, w, h } = readRectFromView(view, rOff, compressed);
		...
		case EMFPLUS_DRAWARC: {
			...
			if (recDataSize < (isFill ? 12 : 8)) return true;
			let aOff = dataOff;
			if (isFill) {
				ctx.fillStyle = resolveBrushPaint(rCtx, recFlags, view.getUint32(aOff, true));
				aOff += 4;
			}
		...
		case EMFPLUS_DRAWLINES:
			if (recDataSize >= 4) {
				...
				if (recFlags & 8192) ctx.closePath();
				ctx.stroke();
			}
			return true;
...
function readPresetColors(view, off, end) {
	...
	if (count === 0 || count > MAX_GRADIENT_ELEMENTS) return null;
	const posOff = off + 4;
	const colOff = posOff + count * 4;
	const next = colOff + count * 4;
	if (next > end) return null;
	const stops = [];
	for (let i = 0; i < count; i++) stops.push({
...
function parseLinearGradient(view, b, end) {
	...
	emfLog(`parseEmfPlusBrushObject: linear gradient (${p1.x.toFixed(1)},${p1.y.toFixed(1)})\u2192(${p2.x.toFixed(1)},${p2.y.toFixed(1)}), ${stops.length} stop(s)`);
	return {
		kind: "plus-brush",
		color: argbToRgba(startArgb),
		gradient: {
...
function parsePathGradient(view, b, end) {
	...
	if (surroundCount > MAX_GRADIENT_ELEMENTS) return {
		kind: "plus-brush",
	...
	if (flags & BRUSH_DATA_PATH) {
		if (o + 4 <= end) {
			...
			if (pathSize > 0 && o + pathSize <= end) {
				...
				if (path) boundaryPts = path.points;
				o += pathSize;
			}
		}
	} else if (o + 4 <= end) {
		...
		if (ptCount > 0 && ptCount <= MAX_GRADIENT_ELEMENTS && o + ptCount * 8 <= end) {
			for (let i = 0; i < ptCount; i++) boundaryPts.push({
	...
	let stops = [{
		...
		offset: 1,
		color: argbToRgba(surroundArgb)
	}];
	if (flags & BRUSH_DATA_PRESET_COLORS) {
		const preset = readPresetColors(view, o, end);
		if (preset) stops = preset.stops.map((s) => ({
			offset: 1 - s.offset,
			color: s.color
		}));
	} else if (flags & BRUSH_DATA_BLEND_FACTORS_H) {
	...
	emfLog(`parseEmfPlusBrushObject: path gradient centre=(${cx.toFixed(1)},${cy.toFixed(1)}), r=${r.toFixed(1)}, ${stops.length} stop(s)`);
	return {
		kind: "plus-brush",
		color: argbToRgba(centerArgb),
		gradient: {
...
function parseEmfPlusBrushObject(view, dataOff, recDataSize) {
	if (recDataSize < 8) return null;
	const end = dataOff + recDataSize;
	const typeOff = dataOff + (looksLikeGraphicsVersion(view.getUint32(dataOff, true)) ? 4 : 0);
	if (typeOff + 8 > end) return null;
	const brushType = view.getUint32(typeOff, true);
	const b = typeOff + 4;
	switch (brushType) {
		case EMFPLUS_BRUSHTYPE_SOLID: return {
			kind: "plus-brush",
			color: argbToRgba(view.getUint32(b, true))
		};
		case EMFPLUS_BRUSHTYPE_HATCHFILL:
			if (b + 8 <= end) return {
				kind: "plus-brush",
				color: argbToRgba(view.getUint32(b + 4, true))
			};
			return {
				kind: "plus-brush",
				color: "rgba(0,0,0,1)"
			};
		case EMFPLUS_BRUSHTYPE_LINEARGRADIENT: return parseLinearGradient(view, b, end) ?? {
			kind: "plus-brush",
			color: "rgba(0,0,0,1)"
		};
		case EMFPLUS_BRUSHTYPE_PATHGRADIENT: return parsePathGradient(view, b, end) ?? {
			kind: "plus-brush",
			color: "rgba(0,0,0,1)"
		};
		default: return {
...
function decodeEmfPlusBitmapPixels(view, pixelStart, width, height, stride, pixelFormat) {
	...
	const bmpRowStride = width * 4 + 3 & -4;
	const pixelDataSize = bmpRowStride * height;
	const bmpData = new Uint8Array(pixelDataSize);
	for (let y = 0; y < height; y++) {
		const rowOff = pixelStart + (topDown ? y : height - 1 - y) * absStride;
...
function parseEmfPlusPenObject(view, dataOff, recDataSize) {
	...
	let dashStyle = 0;
	for (const [flag, size] of flagSizes) if (penFlags & flag) if (flag === 1) brushOff += 24;
	else {
		if (flag === 32 && brushOff + 4 <= dataOff + recDataSize) dashStyle = view.getUint32(brushOff, true);
		brushOff += size;
	}
	if (penFlags & 256) {
...
function parseEmfPlusImageObject(view, dataOff, recDataSize, objectId) {
	...
	if (imgType === 1 && recDataSize >= 28) {
		...
		if (bmpType === 1) {
			...
			emfLog(`  Bitmap(Pixel): ${bmpW}\xD7${bmpH}, stride=${bmpStride}, pixelFormat=0x${pixelFormat.toString(16).padStart(8, "0")}`);
			const pixelStart = dataOff + 28;
			if (bmpW > 0 && bmpH > 0 && bmpW <= 8192 && bmpH <= 8192 && pixelStart + Math.abs(bmpStride) * bmpH <= view.byteLength) {
				const decoded = decodeEmfPlusBitmapPixels(view, pixelStart, bmpW, bmpH, bmpStride, pixelFormat);
				if (decoded) {
			...
			if (imgLen > 0 && imgStart + imgLen <= view.byteLength) {
				imgData = view.buffer.slice(view.byteOffset + imgStart, view.byteOffset + imgStart + imgLen);
				if (imgData.byteLength >= 4) {
					const hdr = new Uint8Array(imgData, 0, 4);
					emfLog(`  Bitmap(Compressed): first 4 bytes = [${Array.from(hdr).map((b) => b.toString(16).padStart(2, "0")).join(" ")}]`);
				}
			}
		}
		...
		if (mfDataSize > 0 && mfStart + mfDataSize <= view.byteLength) {
			imgData = view.buffer.slice(view.byteOffset + mfStart, view.byteOffset + mfStart + mfDataSize);
			if (imgData.byteLength >= 4) new DataView(imgData).getUint32(0, true);
		} else emfWarn(`  Metafile: out of bounds or empty (mfStart=0x${mfStart.toString(16)}, mfDataSize=${mfDataSize}, viewLen=${view.byteLength})`);
	}
	return {
		data: imgData,
		type: imgType
...
function parseEmfPlusFontObject(view, dataOff, recDataSize) {
	...
	if (nameLen > 0 && dataOff + 24 + nameLen * 2 <= dataOff + recDataSize) family = readUtf16LE(view, dataOff + 24, nameLen) || "sans-serif";
	return {
		kind: "plus-font",
		emSize: emSize || 12,
		flags: styleFlags,
...
function handleEmfPlusObjectRecord(rCtx, recFlags, dataOff, recDataSize) {
	...
	switch (recFlags >> 8 & 127) {
		...
		case EMFPLUS_OBJECTTYPE_REGION: {
			...
			if (region) objectTable.set(objectId, region);
			break;
		}
	}
}
...
function parseRegionNode(view, off, endOff, depth = 0) {
	...
	if (depth > MAX_REGION_NODE_DEPTH) return null;
	...
	if (nodeType === 268435458) return {
		node: { type: "empty" },
		bytesRead: 4
	};
	if (nodeType === 268435459) return {
		node: { type: "infinite" },
		bytesRead: 4
	};
...
function parseEmfPlusRegionObject(view, off, maxLen) {
	...
	if (regionNodeCount === 0 || regionNodeCount > 1e5) return null;
	...
	switch (recType) {
			...
			if (recDataSize >= 28) {
				...
				if (strLen > 0 && dataOff + 28 + strLen * 2 <= dataOff + recDataSize) {
					...
					if (text.length > 0 && font && font.kind === "plus-font") {
						...
						if (sf && sf.kind === "plus-stringformat") switch (sf.alignment) {
							case 1:
		...
		case EMFPLUS_DRAWDRIVERSTRING:
			if (recDataSize >= 16) {
				...
				const alignedPosOff = glyphsOff + glyphCount * 2 + 3 & -4;
		...
		case EMFPLUS_DRAWIMAGE:
			if (recDataSize >= 24) {
				...
				emfLog(`DrawImage: imgId=${imgId}, dest=(${dx},${dy},${dw},${dh}), compressed=${compressed}, hasObj=${Boolean(imgObj)}, objKind=${imgObj?.kind}, hasData=${Boolean(hasData)}, dataLen=${hasData ? imgObj.data.byteLength : 0}, isMetafile=${imgObj?.kind === "plus-image" ? imgObj.type === 2 : "N/A"}`);
				emfLog(`DrawImage: worldTransform=[${rCtx.worldTransform.map((v) => v.toFixed(3)).join(", ")}]`);
		...
		case EMFPLUS_DRAWIMAGEPOINTS:
			if (recDataSize >= 28) {
				...
				if (count >= 3 && imgObj && imgObj.kind === "plus-image" && imgObj.data) {
					...
					rCtx.totalDrawImageCalls++;
					emfLog(`DrawImagePoints: imgId=${imgId}, points=[(${p1x},${p1y}),(${p2x},${p2y}),(${p3x},${p3y})], dest=(${dx.toFixed(1)},${dy.toFixed(1)},${dw.toFixed(1)},${dh.toFixed(1)})`);
					emfLog(`DrawImagePoints: worldTransform=[${rCtx.worldTransform.map((v) => v.toFixed(3)).join(", ")}]`);
				...
				} else imgObj && imgObj.kind === "plus-image" && imgObj.data;
			}
			return true;
		default: return false;
	}
	...
	while (offset + 12 <= end && recordCount < maxRecords) {
		...
		switch (recType) {
			...
			case EMFPLUS_ENDOFFILE:
				offset = end;
				continue;
			case EMFPLUS_GETDC: break;
			case EMFPLUS_OBJECT: {
				const isContinuation = (recFlags & 32768) !== 0;
				const objectId = recFlags & 255;
				if (isContinuation) if (rCtx.continuationBuffer === null) {
					if (recDataSize >= 4) {
						...
						if (!Number.isFinite(totalSize) || totalSize <= 0 || totalSize > MAX_CONTINUATION_BYTES || totalSize > remainingEmfPlusBytes || recDataSize - 4 < 0);
							...
							const chunk = new Uint8Array(view.buffer, view.byteOffset + dataOff + 4, Math.min(chunkSize, totalSize));
					...
					const chunk = new Uint8Array(view.buffer, view.byteOffset + dataOff, Math.min(recDataSize, remaining));
					...
					const chunk = new Uint8Array(view.buffer, view.byteOffset + dataOff, Math.min(recDataSize, remaining));
					rCtx.continuationBuffer.set(chunk, rCtx.continuationOffset);
					const completeView = new DataView(rCtx.continuationBuffer.buffer, rCtx.continuationBuffer.byteOffset, rCtx.continuationBuffer.byteLength);
					const assembledFlags = rCtx.continuationObjectType << 8 | objectId;
					handleEmfPlusObjectRecord({
						...rCtx,
						view: completeView
					}, assembledFlags, 0, rCtx.continuationTotalSize);
				...
				} else handleEmfPlusObjectRecord(rCtx, recFlags, dataOff, recDataSize);
				break;
			}
			default:
				if (!(handleEmfPlusDrawRecord(rCtx, recType, recFlags, dataOff, recDataSize) || handleEmfPlusTextImageRecord(rCtx, recType, recFlags, dataOff, recDataSize) || handleEmfPlusStateRecord(rCtx, recType, recFlags, dataOff, recDataSize))) console.warn(`[emf-converter] Unhandled EMF+ record type: 0x${recType.toString(16)}`);
				break;
		}
		offset += recSize;
	}
	if (recordCount >= maxRecords) console.warn(`[emf-converter] EMF+ record limit reached (${maxRecords}). Output may be incomplete.`);
	const summary = [];
	for (const [type, cnt] of emfPlusRecordTypes) summary.push(`${EMFPLUS_REC_NAMES[type] ?? `0x${type.toString(16)}`}:${cnt}`);
	emfLog(`replayEmfPlusRecords: totalImageObjects=${rCtx.totalImageObjects}, totalDrawImageCalls=${rCtx.totalDrawImageCalls}, deferredImages=${rCtx.deferredImages.length}`);
	emfLog(`replayEmfPlusRecords: object table has ${rCtx.objectTable.size} entries: [${Array.from(rCtx.objectTable.entries()).map(([id, obj]) => `${id}:${obj.kind}`).join(", ")}]`);
	if (state) {
...
function replayEmfRecords(view, ctx, bounds, canvasW, canvasH, dpiScale = 1, replayOptions = {}) {
	emfLog(`replayEmfRecords: bounds=(${bounds.left},${bounds.top})\u2192(${bounds.right},${bounds.bottom}), canvas=${canvasW}\xD7${canvasH}`);
	...
	emfLog(`replayEmfRecords: logical=${logicalW}\xD7${logicalH}, scale=(${sx.toFixed(4)},${sy.toFixed(4)})`);
	const rCtx = {
		ctx,
		view,
		objectTable: /* @__PURE__ */ new Map(),
		state: {
			...defaultState(),
			fontFamilyMap: replayOptions.fontFamilyMap
		},
		stateStack: [],
		inPath: false,
		windowOrg: {
	...
	while (offset + 8 <= maxOffset && recordCount < maxRecords) {
		...
		if (recType === EMR_COMMENT) {
			if (recSize >= 16) {
				...
				if (sig === EMFPLUS_SIGNATURE && commentDataSize > 4) {
					emfPlusCommentCount++;
					emfLog(`replayEmfRecords: EMF+ comment #${emfPlusCommentCount} at offset 0x${offset.toString(16)}, dataSize=${commentDataSize}`);
					const deferred = replayEmfPlusRecords(view, dataOff + 8, commentDataSize - 4, ctx, canvasW, canvasH, emfPlusState, dpiScale, maxRecordsEmfPlus, replayOptions.fontFamilyMap);
					emfLog(`replayEmfRecords: EMF+ comment #${emfPlusCommentCount} returned ${deferred.length} deferred images`);
					allDeferredImages.push(...deferred);
				} else if (sig === EMR_COMMENT_PUBLIC_SIGNATURE) emfLog(`replayEmfRecords: EMR_COMMENT_PUBLIC at offset 0x${offset.toString(16)}, size=${commentDataSize}`);
				else emfLog(`replayEmfRecords: EMR_COMMENT (sig=0x${sig.toString(16).padStart(8, "0")}) at offset 0x${offset.toString(16)}, size=${commentDataSize}`);
			}
			offset += recSize;
			continue;
		}
		if (recType === EMR_EOF) {
			const summary = [];
			for (const [type, count] of gdiRecordTypes) summary.push(`${GDI_NAMES[type] ?? `0x${type.toString(16)}`}:${count}`);
			emfLog(`replayEmfRecords: total deferred images = ${allDeferredImages.length}, EMF+ object table size = ${emfPlusState.objectTable.size}`);
			break;
		}
		...
		if (!(handleEmfGdiStateRecord(rCtx, recType, offset, dataOff, recSize) || handleEmfGdiDrawRecord(rCtx, recType, offset, dataOff, recSize) || handleEmfGdiPolyPathRecord(rCtx, recType, offset, dataOff, recSize))) console.warn(`[emf-converter] Unhandled EMR record type: ${recType}`);
		offset += recSize;
	}
	if (recordCount >= maxRecords) console.warn(`[emf-converter] EMF record limit reached (${maxRecords}). Output may be incomplete.`);
	...
	switch (recType) {
		...
		case META_ELLIPSE:
			if (recSize >= 14) {
				...
				const l = view.getInt16(dataOff + 6, true);
				ctx.beginPath();
				ctx.ellipse(mx((l + r) / 2), my((t + b) / 2), Math.abs(mw(r - l)) / 2, Math.abs(mh(b - t)) / 2, 0, 0, Math.PI * 2);
				applyBrush(ctx, state);
				ctx.fill();
				applyPen(ctx, state);
				ctx.stroke();
		...
		case META_CHORD:
			if (recSize >= 22) {
				...
				ctx.ellipse(mx(cxA), my(cyA), Math.abs(mw(rxA)), Math.abs(mh(ryA)), 0, startAngle, endAngle, false);
		...
		case META_POLYPOLYGON:
			if (recSize >= 10) {
				...
				ctx.beginPath();
				for (const count of counts) if (count > 0 && polyOff + count * 4 <= offset + recSize) {
					for (let i = 0; i < count; i++) {
...
function replayWmfRecords(view, ctx, header, canvasW, canvasH, replayOptions = {}) {
	...
	const allocObjectSlot = () => {
		let slot = 0;
		while (objectTable.has(slot)) slot++;
		return slot;
	};
	const state = {
		...defaultState(),
	...
	while (offset + 6 <= maxOffset && recordCount < maxRecords) {
		...
		switch (recType) {
			...
			case META_SETBKMODE:
				...
				break;
			case META_SETROP2:
				if (recSize >= 8) state.rop2 = view.getUint16(dataOff, true);
				break;
	...
	if (recordCount >= maxRecords) console.warn(`[emf-converter] WMF record limit reached (${maxRecords}). Output may be incomplete.`);
}
var MAX_METAFILE_RECURSION = 3;
async function processDeferredImages(ctx, deferredImages, recursionDepth = 0) {
	emfLog(`processDeferredImages: processing ${deferredImages.length} deferred images (recursionDepth=${recursionDepth})...`);
	for (let idx = 0; idx < deferredImages.length; idx++) {
		const img = deferredImages[idx];
		emfLog(`  Deferred image [${idx}]: isMetafile=${img.isMetafile}, dataLen=${img.imageData.byteLength}, dest=(${img.dx.toFixed(1)},${img.dy.toFixed(1)},${img.dw.toFixed(1)},${img.dh.toFixed(1)}), transform=[${img.transform.map((v) => v.toFixed(3)).join(",")}]`);
		try {
			const plainBuffer = new ArrayBuffer(img.imageData.byteLength);
			new Uint8Array(plainBuffer).set(new Uint8Array(img.imageData));
			ctx.setTransform(img.transform[0], img.transform[1], img.transform[2], img.transform[3], img.transform[4], img.transform[5]);
			if (img.isMetafile) {
				if (recursionDepth >= MAX_METAFILE_RECURSION) {
					emfWarn(`  Deferred image [${idx}]: skipping embedded metafile \u2014 recursion depth ${recursionDepth} >= ${MAX_METAFILE_RECURSION}`);
					continue;
				}
				...
				if (metafileDataUrl) {
					emfLog(`  Deferred image [${idx}]: metafile converted, dataUrl length=${metafileDataUrl.length}`);
					...
					emfLog(`  Deferred image [${idx}]: creating ImageBitmap from ${metaBlob.size} byte blob (${mime})...`);
					...
					bitmap.close();
				} else emfWarn(`  Deferred image [${idx}]: metafile conversion returned null`);
			} else {
				emfLog(`  Deferred image [${idx}]: creating ImageBitmap from ${plainBuffer.byteLength} byte blob...`);
			...
			console.warn("[emf-converter] Deferred image draw failed:", imgErr instanceof Error ? imgErr.message : imgErr, `(isMetafile=${img.isMetafile}, dataLen=${img.imageData.byteLength})`);
		}
	}
	ctx.setTransform(1, 0, 0, 1, 0, 0);
}
async function convertEmfToDataUrl(buffer, options, recursionDepth = 0) {
	...
	try {
		...
		emfLog(`Input buffer: ${buffer.byteLength} bytes, maxWidth=${effectiveMaxWidth}, maxHeight=${effectiveMaxHeight}, dpiScale=${dpiScale}`);
		if (buffer.byteLength >= 16) {
			const hdrBytes = new Uint8Array(buffer, 0, 16);
			emfLog(`First 16 bytes: [${Array.from(hdrBytes).map((b) => b.toString(16).padStart(2, "0")).join(" ")}]`);
		}
		...
		const setup = createCanvas(logicalW, logicalH, effectiveMaxWidth, effectiveMaxHeight, dpiScale, opts.maxCanvasDimension);
		if (!setup) {
			emfLog("convertEmfToDataUrl: createCanvas returned null — returning null");
			return null;
		}
		const { canvas, ctx } = setup;
		emfLog(`convertEmfToDataUrl: canvas created ${canvas.width}\xD7${canvas.height} (dpiScale=${dpiScale})`);
		ctx.save();
		...
		emfLog(`convertEmfToDataUrl: replayEmfRecords returned ${deferredImages.length} deferred images`);
		ctx.restore();
		...
		if (result) emfLog(`convertEmfToDataUrl: SUCCESS \u2014 data URL length=${result.length}`);
		else emfWarn("convertEmfToDataUrl: exportCanvasToPngDataUrl returned null");
...
async function convertWmfToDataUrl(buffer, options, recursionDepth = 0) {
	...
	try {
		emfLog("=== convertWmfToDataUrl START ===", `buffer=${buffer.byteLength} bytes, dpiScale=${dpiScale}`);
		const view = new DataView(buffer);
...
//# sourceMappingURL=emf-converter.js.map